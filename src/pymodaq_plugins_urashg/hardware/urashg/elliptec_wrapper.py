...
class ElliptecController:
    ...
    def __init__(self, port: str = '/dev/ttyUSB1', baudrate: int = 9600, 
                 timeout: float = 2.0, mount_addresses: str = '2,3,8', 
        ...
        self.logger.info(f"ElliptecController initialized: port={port}, mounts={mount_addresses}, mock={mock_mode}")
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.mount_addresses:
                ...
                if response:
                    self.logger.info(f"Successfully connected to Elliptec on {self.port}")
                    self._connected = True
                    return True
                else:
        ...
        if response:
            try:
                ...
                if 'PO' in response_str:
                    ...
                    self.logger.debug(f"Mount {mount_address} position: {degrees:.2f}degrees")
                    return degrees
                    
            except Exception as e:
                self.logger.error(f"Error parsing position for mount {mount_address}: {e}")
    ...
    def move_absolute(self, mount_address: str, position_degrees: float) -> bool:
        ...
        try:
            ...
            if response:
                ...
                self.logger.info(f"Mount {mount_address} moved to {position_degrees:.2f}degrees")
                return True
            else:
        ...
        try:
            ...
            with self._lock:
                ...
                if response and len(response) > 0:
                    ...
                    self.logger.error(f"Mount {mount_address} not responding after home command")
                    return False
