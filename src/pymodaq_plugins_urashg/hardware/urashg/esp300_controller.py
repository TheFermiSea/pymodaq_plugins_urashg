...
class ESP300Axis:
    ...
    def get_position(self) -> Optional[float]:
        ...
        try:
            ...
            logger.error(f"Invalid position response for axis {self.axis_number}: {response}")
            return None
    ...
    def is_motion_done(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Invalid motion done response for axis {self.axis_number}: {response}")
            return False
    ...
    def is_enabled(self) -> bool:
        ...
        try:
            ...
            logger.error(f"Invalid enabled response for axis {self.axis_number}: {response}")
            return False
    ...
    def wait_for_stop(self, timeout: float = 30.0, poll_interval: float = 0.1) -> bool:
        ...
        logger.warning(f"Timeout waiting for axis {self.axis_number} motion to complete")
        return False
    ...
    def get_units(self) -> str:
        ...
        try:
            ...
            logger.error(f"Invalid units response for axis {self.axis_number}: {response}")
            return self.config.units
    ...
    def set_software_limits(self, left_limit: float, right_limit: float) -> bool:
        ...
        try:
            ...
            response2 = self._send_command(f"SR{right_limit:.6f}", expect_response=False)
...
class ESP300Controller:
    ...
    def __init__(self, port: str = '/dev/ttyUSB0', baudrate: int = 19200, 
                 timeout: float = 3.0, axes_config: Optional[List[AxisConfig]] = None):
        """
        ...
        logger.info(f"ESP300 controller initialized for {port}, {len(axes_config)} axes")
    ...
    def move_multiple_axes(self, positions: Dict[int, float], wait: bool = True) -> bool:
        """
...
if __name__ == '__main__':
    ...
    if controller.connect():
        ...
        if x_axis:
            print(f"CONFIG: X-axis position: {x_axis.get_position()} {x_axis.get_units()}")
