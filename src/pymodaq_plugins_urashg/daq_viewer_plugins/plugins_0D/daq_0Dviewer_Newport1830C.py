...
class DAQ_0DViewer_Newport1830C(DAQ_Viewer_base):
    ...
    params = comon_parameters + [
        ...
        {'title': 'Connection:', 'name': 'connection_group', 'type': 'group', 'children': [
            {'title': 'Serial Port:', 'name': 'serial_port', 'type': 'str', 'value': '/dev/ttyUSB2'},
            {'title': 'Baudrate:', 'name': 'baudrate', 'type': 'int', 'value': 9600},
            {'title': 'Timeout (s):', 'name': 'timeout', 'type': 'float', 'value': 2.0},
        ]},
        
        # Measurement settings
        {'title': 'Measurement:', 'name': 'measurement_group', 'type': 'group', 'children': [
            {'title': 'Wavelength (nm):', 'name': 'wavelength', 'type': 'float', 
             'value': 800.0, 'min': 400.0, 'max': 1100.0, 'suffix': 'nm'},
            {'title': 'Units:', 'name': 'units', 'type': 'list', 
             'values': ['W', 'dBm'], 'value': 'W'},
            {'title': 'Power Range:', 'name': 'power_range', 'type': 'list',
             'values': ['Auto', 'Range 1', 'Range 2', 'Range 3', 'Range 4', 'Range 5', 'Range 6', 'Range 7'], 
             'value': 'Auto'},
            {'title': 'Filter Speed:', 'name': 'filter_speed', 'type': 'list',
             'values': ['Slow', 'Medium', 'Fast'], 'value': 'Medium'},
            {'title': 'Averaging:', 'name': 'averaging', 'type': 'int', 'value': 3, 'min': 1, 'max': 20},
        ]},
        
        # Calibration actions
        {'title': 'Calibration:', 'name': 'calibration_group', 'type': 'group', 'children': [
            {'title': 'Zero Adjust:', 'name': 'zero_adjust', 'type': 'action'},
        ]},
        
        # Status display
        {'title': 'Status:', 'name': 'status_group', 'type': 'group', 'children': [
            {'title': 'Current Power:', 'name': 'current_power', 'type': 'float', 
             'value': 0.0, 'readonly': True, 'suffix': 'W'},
            {'title': 'Device Status:', 'name': 'device_status', 'type': 'str', 
    ...
    def ini_detector(self, controller=None):
        ...
        try:
            self.emit_status(ThreadCommand('show_splash', 'Initializing Newport 1830-C...'))
    ...
    def _apply_measurement_settings(self):
        ...
        try:
            ...
            # Set power range
            power_range = self.settings.child('measurement_group', 'power_range').value()
            self.controller.set_power_range(power_range)
            
            # Set filter speed
            filter_speed = self.settings.child('measurement_group', 'filter_speed').value()
            ...
            self.emit_status(ThreadCommand('Update_Status', [f'Error applying settings: {e}']))
    ...
    def close(self):
        ...
        try:
            if self.controller:
                self.controller.disconnect()
                self.controller = None
            self.settings.child('status_group', 'device_status').setValue('Disconnected')
            self.emit_status(ThreadCommand('Update_Status', ['Newport 1830-C connection closed']))
        except Exception as e:
    ...
    def grab_data(self, Naverage=1, **kwargs):
        ...
        try:
            ...
            if not readings:
                ...
                power_value = 0.0
                self.emit_status(ThreadCommand('Update_Status', ['No valid power readings']))
            else:
                # Calculate average
                power_value = sum(readings) / len(readings)
                
                # Update status display
                self.settings.child('status_group', 'current_power').setValue(power_value)
            ...
            self.emit_status(ThreadCommand('Update_Status', [f'Error during measurement: {e}']))
    ...
    def commit_settings(self, param):
        ...
        try:
            ...
            if param_name in ['wavelength', 'units', 'power_range', 'filter_speed']:
                self._apply_measurement_settings()
                self.emit_status(ThreadCommand('Update_Status', [f'Updated {param_name}']))
                
            elif param_name == 'zero_adjust':
                self._perform_zero_adjust()
                
        except Exception as e:
            self.emit_status(ThreadCommand('Update_Status', [f'Error updating settings: {e}']))

    def _perform_zero_adjust(self):
        """Perform zero adjustment on the power meter."""
        try:
            if not self.controller or not self.controller.is_connected():
                self.emit_status(ThreadCommand('Update_Status', ['Power meter not connected']))
                return
                
            self.emit_status(ThreadCommand('Update_Status', ['Performing zero adjustment...']))
            
            if self.controller.zero_adjust():
                self.emit_status(ThreadCommand('Update_Status', ['Zero adjustment completed successfully']))
            else:
                self.emit_status(ThreadCommand('Update_Status', ['Zero adjustment failed']))
                
        except Exception as e:
            self.emit_status(ThreadCommand('Update_Status', [f'Error during zero adjustment: {e}']))
