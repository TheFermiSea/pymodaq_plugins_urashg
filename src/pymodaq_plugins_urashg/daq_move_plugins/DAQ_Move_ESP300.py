...
class DAQ_Move_ESP300(DAQ_Move_base):
    ...
    params = comon_parameters_fun(is_multiaxes=True, axis_names=_axis_names, epsilon=_epsilon) + [
        ...
        {'title': 'Connection:', 'name': 'connection_group', 'type': 'group', 'children': [
            {'title': 'Serial Port:', 'name': 'serial_port', 'type': 'str', 'value': '/dev/ttyUSB3'},
            {'title': 'Baudrate:', 'name': 'baudrate', 'type': 'list', 
             'values': [9600, 19200, 38400, 57600, 115200], 'value': 19200},
            {'title': 'Timeout (s):', 'name': 'timeout', 'type': 'float', 'value': 3.0},
            {'title': 'Mock Mode:', 'name': 'mock_mode', 'type': 'bool', 'value': False},
        ]},
        
        # Axes configuration
        {'title': 'Axes Config:', 'name': 'axes_config', 'type': 'group', 'children': [
            {'title': 'Number of Axes:', 'name': 'num_axes', 'type': 'int', 'value': 3, 'min': 1, 'max': 3},
            {'title': 'Axis 1:', 'name': 'axis1_group', 'type': 'group', 'children': [
                {'title': 'Name:', 'name': 'axis1_name', 'type': 'str', 'value': 'X Stage'},
                {'title': 'Units:', 'name': 'axis1_units', 'type': 'list', 
                 'values': ['millimeter', 'micrometer', 'degree', 'encoder count'], 'value': 'millimeter'},
                {'title': 'Software Limits:', 'name': 'axis1_limits', 'type': 'bool', 'value': False},
                {'title': 'Min Limit:', 'name': 'axis1_min', 'type': 'float', 'value': -25.0, 'suffix': 'mm'},
                {'title': 'Max Limit:', 'name': 'axis1_max', 'type': 'float', 'value': 25.0, 'suffix': 'mm'},
            ]},
            {'title': 'Axis 2:', 'name': 'axis2_group', 'type': 'group', 'children': [
                {'title': 'Name:', 'name': 'axis2_name', 'type': 'str', 'value': 'Y Stage'},
                {'title': 'Units:', 'name': 'axis2_units', 'type': 'list', 
                 'values': ['millimeter', 'micrometer', 'degree', 'encoder count'], 'value': 'millimeter'},
                {'title': 'Software Limits:', 'name': 'axis2_limits', 'type': 'bool', 'value': False},
                {'title': 'Min Limit:', 'name': 'axis2_min', 'type': 'float', 'value': -25.0, 'suffix': 'mm'},
                {'title': 'Max Limit:', 'name': 'axis2_max', 'type': 'float', 'value': 25.0, 'suffix': 'mm'},
            ]},
            {'title': 'Axis 3:', 'name': 'axis3_group', 'type': 'group', 'children': [
                {'title': 'Name:', 'name': 'axis3_name', 'type': 'str', 'value': 'Z Focus'},
                {'title': 'Units:', 'name': 'axis3_units', 'type': 'list', 
                 'values': ['millimeter', 'micrometer', 'degree', 'encoder count'], 'value': 'micrometer'},
                {'title': 'Software Limits:', 'name': 'axis3_limits', 'type': 'bool', 'value': False},
                {'title': 'Min Limit:', 'name': 'axis3_min', 'type': 'float', 'value': -1000.0, 'suffix': 'μm'},
                {'title': 'Max Limit:', 'name': 'axis3_max', 'type': 'float', 'value': 1000.0, 'suffix': 'μm'},
            ]},
        ]},
        
        # Motion settings
        {'title': 'Motion:', 'name': 'motion_group', 'type': 'group', 'children': [
            {'title': 'Home on Init:', 'name': 'home_on_init', 'type': 'bool', 'value': False},
            {'title': 'Home Type:', 'name': 'home_type', 'type': 'list', 
             'values': list(range(7)), 'value': 1},
            {'title': 'Wait for Motion:', 'name': 'wait_motion', 'type': 'bool', 'value': True},
            {'title': 'Motion Timeout (s):', 'name': 'motion_timeout', 'type': 'float', 'value': 30.0},
            {'title': 'Position Poll Rate (Hz):', 'name': 'poll_rate', 'type': 'float', 'value': 10.0},
        ]},
        ...
        {'title': 'Status:', 'name': 'status_group', 'type': 'group', 'children': [
            {'title': 'Connection:', 'name': 'connection_status', 'type': 'str', 'value': 'Disconnected', 'readonly': True},
            {'title': 'Axis 1 Position:', 'name': 'axis1_position', 'type': 'float', 'value': 0.0, 'readonly': True, 'suffix': 'mm'},
            {'title': 'Axis 2 Position:', 'name': 'axis2_position', 'type': 'float', 'value': 0.0, 'readonly': True, 'suffix': 'mm'},
            {'title': 'Axis 3 Position:', 'name': 'axis3_position', 'type': 'float', 'value': 0.0, 'readonly': True, 'suffix': 'μm'},
            {'title': 'Last Error:', 'name': 'last_error', 'type': 'str', 'value': 'None', 'readonly': True},
        ]},
    ]
    ...
    def ini_stage(self, controller=None):
        ...
        try:
            ...
            if self.settings.child('connection_group', 'mock_mode').value():
                ...
                self.emit_status(ThreadCommand('Update_Status', ['ESP300 in mock mode']))
            ...
            if errors:
                self.emit_status(ThreadCommand('Update_Status', [f'Cleared {len(errors)} errors']))
            ...
            if not self.controller.enable_all_axes():
                self.emit_status(ThreadCommand('Update_Status', ['Warning: Some axes failed to enable']))
    ...
    def _build_axes_config(self, num_axes: int) -> List[AxisConfig]:
        ...
        for i in range(num_axes):
            ...
            name = self.settings.child('axes_config', group_name, f'axis{axis_num}_name').value()
            units = self.settings.child('axes_config', group_name, f'axis{axis_num}_units').value()
            
            config = AxisConfig(axis_num, name, units)
            
            # Set software limits if enabled
            if self.settings.child('axes_config', group_name, f'axis{axis_num}_limits').value():
                config.left_limit = self.settings.child('axes_config', group_name, f'axis{axis_num}_min').value()
                config.right_limit = self.settings.child('axes_config', group_name, f'axis{axis_num}_max').value()
        ...
        try:
            for axis_num, axis in self.controller.axes.items():
                ...
                if not axis.set_units(axis.config.units):
                    self.emit_status(ThreadCommand('Update_Status', [f'Warning: Failed to set units for axis {axis_num}']))
                
                # Set software limits if configured
                if axis.config.left_limit is not None and axis.config.right_limit is not None:
                    if not axis.set_software_limits(axis.config.left_limit, axis.config.right_limit):
                        self.emit_status(ThreadCommand('Update_Status', [f'Warning: Failed to set limits for axis {axis_num}']))
                        
        except Exception as e:
            self.emit_status(ThreadCommand('Update_Status', [f'Error configuring axes: {e}']))
    ...
    def _update_status_display(self):
        """Update status display parameters."""
        try:
            ...
            self.emit_status(ThreadCommand('Update_Status', [f'Error updating status: {e}']))
    ...
    def close(self):
        ...
        try:
            if self.controller:
                ...
                self.controller = None
            self.settings.child('status_group', 'connection_status').setValue('Disconnected')
            self.emit_status(ThreadCommand('Update_Status', ['ESP300 connection closed']))
        except Exception as e:
            self.emit_status(ThreadCommand('Update_Status', [f'Error closing connection: {e}']))
    ...
    def get_actuator_value(self) -> Union[float, List[float]]:
        ...
        try:
            ...
            self.emit_status(ThreadCommand('Update_Status', [f'Error reading positions: {e}']))
    ...
    def move_abs(self, position: Union[float, List[float]]):
        ...
        try:
            if not self.controller or not self.controller.is_connected():
                self.emit_status(ThreadCommand('Update_Status', ['Cannot move: ESP300 not connected']))
                return
            ...
            if self.is_multiaxes:
                ...
                wait_for_motion = self.settings.child('motion_group', 'wait_motion').value()
                if not self.controller.move_multiple_axes(target_positions, wait=wait_for_motion):
                ...
                if self.settings.child('motion_group', 'wait_motion').value():
                    timeout = self.settings.child('motion_group', 'motion_timeout').value()
    ...
    def move_rel(self, position: Union[float, List[float]]):
        ...
        try:
            ...
            if self.is_multiaxes:
                ...
                target_positions = []
                for i, (current, relative) in enumerate(zip(current_positions, position)):
                    target_positions.append(current + relative)
                    
                self.move_abs(target_positions)
            else:
                target_position = current_positions + position
                self.move_abs(target_position)
                
        except Exception as e:
            self.emit_status(ThreadCommand('Update_Status', [f'Relative move error: {e}']))
            self.settings.child('status_group', 'last_error').setValue(str(e))
    ...
    def move_home(self):
        ...
        try:
            if not self.controller or not self.controller.is_connected():
                self.emit_status(ThreadCommand('Update_Status', ['Cannot home: ESP300 not connected']))
                return
        ...
        try:
            if self.controller and self.controller.is_connected():
                if self.controller.stop_all_axes():
                    self.emit_status(ThreadCommand('Update_Status', ['All motion stopped']))
                else:
                    self.emit_status(ThreadCommand('Update_Status', ['Stop command failed']))
            else:
                self.emit_status(ThreadCommand('Update_Status', ['ESP300 not connected']))
            ...
            if param_name == 'home_all':
                ...
                if self.controller and self.controller.is_connected():
                    if self.controller.enable_all_axes():
                        self.emit_status(ThreadCommand('Update_Status', ['All axes enabled']))
                    else:
                        self.emit_status(ThreadCommand('Update_Status', ['Enable failed']))
                        
            elif param_name == 'disable_all':
                if self.controller and self.controller.is_connected():
                    if self.controller.disable_all_axes():
                        self.emit_status(ThreadCommand('Update_Status', ['All axes disabled']))
                    else:
                        self.emit_status(ThreadCommand('Update_Status', ['Disable failed']))
                ...
                if self.controller and self.controller.is_connected():
                    ...
                    if errors:
                        self.emit_status(ThreadCommand('Update_Status', [f'Cleared {len(errors)} errors']))
                        self.settings.child('status_group', 'last_error').setValue('Cleared')
                    else:
                        self.emit_status(ThreadCommand('Update_Status', ['No errors to clear']))
                        self.settings.child('status_group', 'last_error').setValue('None')
    ...
    def check_position(self) -> bool:
        ...
        try:
            ...
            self.emit_status(ThreadCommand('Update_Status', [f'Position check error: {e}']))
            return False
