...
def inverse_malus_law(power, amplitude, phase, offset):
    ...
    if power < offset or power > (amplitude + offset):
        raise ValueError(f"Power {power} outside valid range [{offset}, {amplitude + offset}]")
...
class ElliptecCalibrationExperiment(URASHGBaseExperiment):
    ...
    def __init__(self, dashboard=None):
        ...
        self.rotators = {
            ...
            'Q800': {'type': 'QWP', 'wavelength': 800, 'description': 'Quarter-wave plate'}
        }
            ...
            'children': [
                {
                    ...
                    'type': 'group',  
                    'children': [
                        {'name': 'home_on_start', 'type': 'bool', 'value': True,
                         'tip': 'Home all rotators before calibration'},
                        {'name': 'homing_timeout', 'type': 'float', 'value': 30.0,
                         'limits': [5, 120], 'suffix': 's'},
                        {'name': 'verify_homed_position', 'type': 'bool', 'value': True},
                        {'name': 'position_tolerance', 'type': 'float', 'value': 0.1,
                         'limits': [0.01, 1.0], 'suffix': '°'},
                    ]
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'angle_range', 'type': 'float', 'value': 360.0,
                         'limits': [180, 720], 'suffix': '°'},
                        {'name': 'angle_step', 'type': 'float', 'value': 5.0,
                         'limits': [1, 45], 'suffix': '°'},
                        {'name': 'settling_time', 'type': 'float', 'value': 0.5,
                         'limits': [0.1, 5.0], 'suffix': 's'},
                        {'name': 'averaging_samples', 'type': 'int', 'value': 10,
                         'limits': [1, 100]},
                    ]
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'enable_cross_calibration', 'type': 'bool', 'value': True},
                        {'name': 'test_angles', 'type': 'str', 
                         'value': '0, 45, 90, 135, 180, 225, 270, 315',
                         'tip': 'Comma-separated test angles for cross-calibration'},
                        {'name': 'cross_angle_step', 'type': 'float', 'value': 15.0,
                         'limits': [5, 90], 'suffix': '°'},
                    ]
                },
                {
                    ...
                    'children': [
                        ...
                        {'name': 'wavelength_range', 'type': 'group',
                         'children': [
                             ...
                             {'name': 'step_nm', 'type': 'float', 'value': 25.0,
                              'limits': [5, 100], 'suffix': 'nm'},
                         ]},
                        {'name': 'test_polarizations', 'type': 'str',
                         'value': '0, 90, 180, 270',
                ...
                },
                {
                    ...
                    'children': [
                        {'name': 'fit_function', 'type': 'list',
                         'limits': ['malus_law', 'cosine_squared'], 'value': 'malus_law'},
                        {'name': 'initial_guess_amplitude', 'type': 'float', 'value': 1.0,
                         'limits': [0.1, 10.0]},
                        {'name': 'initial_guess_phase', 'type': 'float', 'value': 0.0,
                         'limits': [-180, 180], 'suffix': '°'},
                        {'name': 'initial_guess_offset', 'type': 'float', 'value': 0.1,
                         'limits': [0.0, 1.0]},
                        {'name': 'max_iterations', 'type': 'int', 'value': 1000,
                         'limits': [100, 10000]},
                        {'name': 'tolerance', 'type': 'float', 'value': 1e-8,
                         'limits': [1e-12, 1e-4]},
                    ]
                },
                {
    ...
    def initialize_specific_hardware(self):
        ...
        for rotator_name, rotator_info in self.rotators.items():
            try:
                ...
                logger.info(f"{rotator_name} ({rotator_info['description']}) at {current_pos}°")
    ...
    def validate_parameters(self):
        ...
        angle_range = self.settings.child('elliptec_calibration', 'individual_calibration', 'angle_range').value()
        angle_step = self.settings.child('elliptec_calibration', 'individual_calibration', 'angle_step').value()
        ...
        if self.settings.child('elliptec_calibration', 'wavelength_calibration', 'enable_wavelength_cal').value():
            wl_start = self.settings.child('elliptec_calibration', 'wavelength_calibration', 'wavelength_range', 'start_nm').value()
            wl_stop = self.settings.child('elliptec_calibration', 'wavelength_calibration', 'wavelength_range', 'stop_nm').value()
            wl_step = self.settings.child('elliptec_calibration', 'wavelength_calibration', 'wavelength_range', 'step_nm').value()
    ...
    def create_data_structures(self):
        ...
        total_cross_steps = 0
        if self.settings.child('elliptec_calibration', 'cross_calibration', 'enable_cross_calibration').value():
            test_angles_str = self.settings.child('elliptec_calibration', 'cross_calibration', 'test_angles').value()
            test_angles = [float(x.strip()) for x in test_angles_str.split(',')]
            cross_step = self.settings.child('elliptec_calibration', 'cross_calibration', 'cross_angle_step').value()
        ...
        if self.settings.child('elliptec_calibration', 'wavelength_calibration', 'enable_wavelength_cal').value():
            ...
            test_pol_str = self.settings.child('elliptec_calibration', 'wavelength_calibration', 'test_polarizations').value()
            test_polarizations = [float(x.strip()) for x in test_pol_str.split(',')]
            total_wavelength_steps = len(wavelengths) * len(test_polarizations) * len(self.rotators)
        
        # Add homing and validation steps
        homing_steps = len(self.rotators) if self.settings.child('elliptec_calibration', 'homing_settings', 'home_on_start').value() else 0
        validation_steps = 0
        if self.settings.child('elliptec_calibration', 'validation', 'validate_calibration').value():
            val_angles_str = self.settings.child('elliptec_calibration', 'validation', 'validation_angles').value()
            val_angles = [float(x.strip()) for x in val_angles_str.split(',')]
            repeatability = self.settings.child('elliptec_calibration', 'validation', 'repeatability_tests').value()
            validation_steps = len(val_angles) * len(self.rotators) * repeatability
        
        self.total_steps = homing_steps + total_individual_steps + total_cross_steps + total_wavelength_steps + validation_steps
        
        logger.info(f"Created data structures: {len(self.rotators)} rotators, {len(angles)} angles per rotator")
    ...
    def run_experiment(self):
        ...
        try:
            ...
            if self.settings.child('elliptec_calibration', 'homing_settings', 'home_on_start').value():
    ...
    def home_all_rotators(self, current_step):
        ...
        timeout = self.settings.child('elliptec_calibration', 'homing_settings', 'homing_timeout').value()
        verify_position = self.settings.child('elliptec_calibration', 'homing_settings', 'verify_homed_position').value()
        tolerance = self.settings.child('elliptec_calibration', 'homing_settings', 'position_tolerance').value()
            ...
            try:
                ...
                if verify_position:
                    ...
                    if abs(pos_value) > tolerance:
                        raise ExperimentError(f"{rotator_name} not at home position: {pos_value}°")
    ...
    def calibrate_individual_rotators(self, current_step):
        ...
        angle_range = self.settings.child('elliptec_calibration', 'individual_calibration', 'angle_range').value()
        angle_step = self.settings.child('elliptec_calibration', 'individual_calibration', 'angle_step').value()
        settling_time = self.settings.child('elliptec_calibration', 'individual_calibration', 'settling_time').value()
        averaging_samples = self.settings.child('elliptec_calibration', 'individual_calibration', 'averaging_samples').value()
        ...
        for rotator_name in self.rotators.keys():
            ...
            self.individual_calibrations[rotator_name]['timestamps'] = np.array(timestamps)
    ...
    def fit_individual_rotator(self, rotator_name):
        ...
        amplitude_guess = self.settings.child('elliptec_calibration', 'fitting_settings', 'initial_guess_amplitude').value()
        phase_guess = self.settings.child('elliptec_calibration', 'fitting_settings', 'initial_guess_phase').value()
        offset_guess = self.settings.child('elliptec_calibration', 'fitting_settings', 'initial_guess_offset').value()
        ...
        try:
            ...
            popt, pcov = curve_fit(
                malus_law, angles, powers, 
                p0=initial_guess,
                maxfev=self.settings.child('elliptec_calibration', 'fitting_settings', 'max_iterations').value()
            )
            ...
            r_squared = 1 - np.sum(residuals**2) / np.sum((powers - np.mean(powers))**2)
            ...
            logger.info(f"{rotator_name} Malus law fit: A={amplitude:.4f}±{amplitude_err:.4f}, "
                       f"φ={phase:.2f}±{phase_err:.2f}°, C={offset:.4f}±{offset_err:.4f}")
            logger.info(f"{rotator_name} fit quality: R²={r_squared:.4f}, "
            ...
            self.individual_calibrations[rotator_name]['fit_params'] = {}
            self.individual_calibrations[rotator_name]['fit_quality'] = {'error': str(e)}
    ...
    def validate_calibration_accuracy(self, current_step):
        ...
        repeatability_tests = self.settings.child('elliptec_calibration', 'validation', 'repeatability_tests').value()
        max_residual_percent = self.settings.child('elliptec_calibration', 'validation', 'max_residual_percent').value()
        ...
        for rotator_name in self.rotators.keys():
            ...
            if 'fit_params' not in self.individual_calibrations[rotator_name]:
                logger.warning(f"No fit parameters for {rotator_name}, skipping validation")
                continue
            ...
            for val_angle in validation_angles:
                ...
                predicted_power = malus_law(val_angle, 
                ...
                residual_percent = abs(mean_measured - predicted_power) / predicted_power * 100
                ...
                logger.debug(f"{rotator_name} validation at {val_angle}°: "
            ...
            self.individual_calibrations[rotator_name]['validation_results'] = {
                ...
                'summary': {
                    ...
                    'mean_error_percent': np.mean([r['residual_percent'] for r in validation_results]),
                    'max_error_percent': np.max([r['residual_percent'] for r in validation_results]),
                    'mean_repeatability_percent': np.mean([r['repeatability_percent'] for r in validation_results])
                }
            }
            
            summary = self.individual_calibrations[rotator_name]['validation_results']['summary']
            logger.info(f"{rotator_name} validation: {summary['passed_points']}/{summary['total_points']} points passed")
            logger.info(f"{rotator_name} mean error: {summary['mean_error_percent']:.2f}%, "
    ...
    def generate_calibration_tables(self):
        ...
        for rotator_name in self.rotators.keys():
            ...
            try:
                angle_interpolator = interp1d(powers_hr, angles_hr, 
                ...
                self.calibration_tables[rotator_name] = {
                    ...
                    'power_to_angle_func': lambda p: inverse_malus_law(p, 
                                                                      fit_params['amplitude'],
                                                                      fit_params['phase'],
                                                                      fit_params['offset'])
                }
                ...
                logger.warning(f"Failed to create interpolation for {rotator_name}: {e}")
        ...
        try:
            with h5py.File(self.data_filepath, 'w') as f:
                ...
                for rotator_name, calib_data in self.individual_calibrations.items():
                    ...
                    rotator_group.create_dataset('timestamps', data=calib_data['timestamps'])
                    ...
                    if 'validation_results' in calib_data and calib_data['validation_results']:
                        val_group = rotator_group.create_group('validation')
                        
                        # Save validation points
                        for i, point in enumerate(calib_data['validation_results']['validation_points']):
                        ...
                        for key, value in calib_data['validation_results']['summary'].items():
                ...
                if self.cross_calibrations:
                    ...
                    for key, cross_data in self.cross_calibrations.items():
                        ...
                        cross_subgroup.attrs['reference_angle'] = cross_data['reference_angle']
                        cross_subgroup.create_dataset('q800_angles', data=cross_data['q800_angles'])
                        cross_subgroup.create_dataset('powers', data=cross_data['powers'])
                    ...
                    for wavelength, wl_data in self.wavelength_calibrations.items():
                        ...
                        for rotator_name, rotator_data in wl_data.items():
                            ...
                            for pol_angle, power in rotator_data.items():
                                rotator_subgroup.attrs[f'pol_{pol_angle:.0f}deg'] = power
                ...
                if hasattr(self, 'calibration_tables'):
                    ...
                    for rotator_name, table_data in self.calibration_tables.items():
                        ...
                        table_subgroup.create_dataset('angles', data=table_data['angles'])
                        table_subgroup.create_dataset('powers', data=table_data['powers'])
    ...
    def get_calibrated_angle_for_power(self, rotator_name, target_power):
        ...
        """
        if not hasattr(self, 'calibration_tables') or rotator_name not in self.calibration_tables:
            raise ExperimentError(f"No calibration available for {rotator_name}")
        
        try:
            power_to_angle_func = self.calibration_tables[rotator_name]['power_to_angle_func']
