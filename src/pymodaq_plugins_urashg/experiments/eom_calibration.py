...
class EOMCalibrationExperiment(URASHGBaseExperiment):
        ...
        eom_params = {
            ...
            'children': [
                ...
                },
                {
                    ...
                    'type': 'group', 
                    'children': [
                        {'name': 'min_voltage', 'type': 'float', 'value': 0.0,
                         'limits': [0, 5], 'suffix': 'V'},
                        {'name': 'max_voltage', 'type': 'float', 'value': 8.0,
                         'limits': [3, 10], 'suffix': 'V'},
                        {'name': 'voltage_steps', 'type': 'int', 'value': 50,
                         'limits': [10, 200]},
                    ]
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'min_power', 'type': 'float', 'value': 0.001,
                         'limits': [0.0001, 0.01], 'suffix': 'W'},
                        {'name': 'max_power', 'type': 'float', 'value': 0.020,
                         'limits': [0.005, 0.1], 'suffix': 'W'},
                        {'name': 'power_points', 'type': 'int', 'value': 10,
                         'limits': [3, 50]},
                ...
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'kp', 'type': 'float', 'value': 24.0,
                         'limits': [0, 100], 'tip': 'Proportional gain'},
                        {'name': 'ki', 'type': 'float', 'value': 3.0,
                         'limits': [0, 50], 'tip': 'Integral gain'},
                        {'name': 'kd', 'type': 'float', 'value': 0.0125,
                         'limits': [0, 1], 'tip': 'Derivative gain'},
                        {'name': 'tolerance', 'type': 'float', 'value': 0.001,
                         'limits': [0.0001, 0.01], 'suffix': '%',
                         'tip': 'Power control tolerance'},
                        {'name': 'max_iterations', 'type': 'int', 'value': 100,
                         'limits': [10, 1000]},
                        {'name': 'settling_time', 'type': 'float', 'value': 0.1,
                         'limits': [0.01, 2.0], 'suffix': 's'},
                    ]
                },
                {
                    'name': 'validation',
                    ...
                    'children': [
                        ...
                        {'name': 'validation_points', 'type': 'int', 'value': 5,
                         'limits': [3, 20]},
                        {'name': 'repeatability_tests', 'type': 'int', 'value': 3,
                         'limits': [1, 10]},
                        {'name': 'max_error_percent', 'type': 'float', 'value': 1.0,
                         'limits': [0.1, 10], 'suffix': '%'},
                    ]
                }
            ]
        }
    ...
    def validate_parameters(self):
        ...
        wl_start = self.settings.child('eom_calibration', 'wavelength_range', 'start_nm').value()
        wl_stop = self.settings.child('eom_calibration', 'wavelength_range', 'stop_nm').value()
        wl_step = self.settings.child('eom_calibration', 'wavelength_range', 'step_nm').value()
        ...
        # Check voltage range
        v_min = self.settings.child('eom_calibration', 'voltage_sweep', 'min_voltage').value()
        v_max = self.settings.child('eom_calibration', 'voltage_sweep', 'max_voltage').value()
        
        if v_min >= v_max:
            raise ExperimentError("Minimum voltage must be less than maximum voltage")
            
        # Check power range
        p_min = self.settings.child('eom_calibration', 'power_control', 'min_power').value()
        p_max = self.settings.child('eom_calibration', 'power_control', 'max_power').value()
    ...
    def create_data_structures(self):
        ...
        v_min = self.settings.child('eom_calibration', 'voltage_sweep', 'min_voltage').value()
        v_max = self.settings.child('eom_calibration', 'voltage_sweep', 'max_voltage').value()
        v_steps = self.settings.child('eom_calibration', 'voltage_sweep', 'voltage_steps').value()
        
        p_min = self.settings.child('eom_calibration', 'power_control', 'min_power').value()
        p_max = self.settings.child('eom_calibration', 'power_control', 'max_power').value()
        p_points = self.settings.child('eom_calibration', 'power_control', 'power_points').value()
        log_spacing = self.settings.child('eom_calibration', 'power_control', 'logarithmic_spacing').value()
        ...
        logger.info(f"Created data structures for {len(wavelengths)} wavelengths, "
    ...
    def run_experiment(self):
        ...
        try:
            ...
            if self.settings.child('eom_calibration', 'validation', 'validate_calibration').value():
    ...
    def run_voltage_sweep(self):
        ...
        for wl_idx, wavelength in enumerate(wavelengths):
            ...
            for v_idx, voltage in enumerate(voltages):
                ...
                # Wait for settling
                settling_time = self.settings.child('eom_calibration', 'pid_settings', 'settling_time').value()
                ...
                # Store results
                self.voltage_sweep_data['measured_power'][wl_idx, v_idx] = measured_power
                self.voltage_sweep_data['photodiode_voltage'][wl_idx, v_idx] = pd_voltage
                ...
                logger.debug(f"Voltage {voltage:.2f}V -> Power {measured_power:.6f}W, PD {pd_voltage:.4f}V")
    ...
    def run_power_control(self):
        ...
        for wl_idx, wavelength in enumerate(wavelengths):
            ...
            for p_idx, target_power in enumerate(target_powers):
                ...
                # Store results
                self.power_control_data['achieved_power'][wl_idx, p_idx] = result['achieved_power']
                self.power_control_data['eom_voltage'][wl_idx, p_idx] = result['eom_voltage']
                self.power_control_data['photodiode_voltage'][wl_idx, p_idx] = result['pd_voltage']
                self.power_control_data['control_iterations'][wl_idx, p_idx] = result['iterations']
                self.power_control_data['control_time'][wl_idx, p_idx] = control_time
                
                # Update progress
                voltage_steps = len(self.voltage_sweep_data['voltages'])
                step = len(wavelengths) * voltage_steps + wl_idx * len(target_powers) + p_idx + 1
                progress = 50 + (step / self.total_steps) * 50  # Second 50% for power control
                self.update_progress(progress)
                
                logger.info(f"Target {target_power:.6f}W -> Achieved {result['achieved_power']:.6f}W "
    ...
    def set_wavelength_and_wait(self, wavelength):
        ...
        self.log_status(f"Set wavelength to {wavelength:.1f} nm, waiting {stabilization_time:.0f}s for stabilization")
    ...
    def control_power_to_target(self, target_power, wavelength):
        ...
        tolerance = self.settings.child('eom_calibration', 'pid_settings', 'tolerance').value()
        max_iterations = self.settings.child('eom_calibration', 'pid_settings', 'max_iterations').value()
        settling_time = self.settings.child('eom_calibration', 'pid_settings', 'settling_time').value()
        ...
        for iteration in range(max_iterations):
            ...
            voltage_adjustment = self.pid_controller.update(target_pd_voltage, current_pd_voltage)
    ...
    def generate_calibration_tables(self):
        ...
        try:
            self.eom_voltage_interpolator = interp2d(
                wavelengths, target_powers, eom_voltages.T, 
        ...
        try:
            self.pd_voltage_interpolator = interp2d(
                wavelengths, target_powers, pd_voltages.T,
                kind='linear', bounds_error=False, fill_value=np.nan
            )
    ...
    def validate_calibration(self):
        ...
        validation_points = self.settings.child('eom_calibration', 'validation', 'validation_points').value()
        repeatability_tests = self.settings.child('eom_calibration', 'validation', 'repeatability_tests').value()
        max_error_percent = self.settings.child('eom_calibration', 'validation', 'max_error_percent').value()
        
        # Generate random test points within calibration range
        wavelengths = self.calibration_ranges['wavelength_min'] + \
                     np.random.random(validation_points) * \
                     (self.calibration_ranges['wavelength_max'] - self.calibration_ranges['wavelength_min'])
        
        powers = self.calibration_ranges['power_min'] + \
                np.random.random(validation_points) * \
        ...
        for wl, target_power in zip(wavelengths, powers):
            ...
            logger.info(f"Validation: Target {target_power:.6f}W -> "
        ...
        logger.info(f"Validation summary: {passed_count}/{len(validation_results)} points passed")
        logger.info(f"Mean error: {np.mean(errors):.2f}%, Max error: {np.max(errors):.2f}%")
    ...
    def get_calibrated_voltage(self, wavelength, power):
        ...
        """
        if not hasattr(self, 'eom_voltage_interpolator') or self.eom_voltage_interpolator is None:
            raise ExperimentError("EOM calibration not available")
            
        # Check if values are within calibration range
        if not (self.calibration_ranges['wavelength_min'] <= wavelength <= self.calibration_ranges['wavelength_max']):
            logger.warning(f"Wavelength {wavelength} nm outside calibrated range")
            
        if not (self.calibration_ranges['power_min'] <= power <= self.calibration_ranges['power_max']):
