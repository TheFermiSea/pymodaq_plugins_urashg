...
class PDSHGExperiment(URASHGBaseExperiment):
            ...
            "children": [
                ...
                },
                {
                    ...
                    "children": [
                        ...
                        {
                            "name": "eom_calibration_file",
                            "type": "browsepath",
                            "value": "",
                        },
                        {
                            "name": "power_tolerance",
                            "type": "float",
                            "value": 1.0,
                            "limits": [0.1, 10.0],
                            "suffix": "%",
                    ...
                    "children": [
                        {
                            ...
                            "value": 100,
                            "limits": [10, 1000],
                            "tip": "Photodiode averaging samples",
                        },
                        {
                ...
                },
                {
                    ...
                    "children": [
                        ...
                        {
                            "name": "expected_shg_exponent",
                            "type": "float",
                            "value": 2.0,
                            "limits": [1.5, 3.0],
                        },
                        {
                            "name": "fit_quality_threshold",
                            "type": "float",
                            "value": 0.95,
                            "limits": [0.5, 1.0],
                            "tip": "Minimum R² for acceptable fits",
                        },
                    ],
                },
                {
                    ...
                    "children": [
                        ...
                        {
                            "name": "polarization_config",
                            "type": "str",
                            "value": "Standard",
                        },
                        {"name": "environmental_notes", "type": "str", "value": ""},
                        {
                            "name": "expected_signal_level",
                            "type": "float",
                            "value": 1.0,
                            "limits": [0.001, 1000],
                            "suffix": "V",
    ...
    def initialize_specific_hardware(self):
        ...
        if self.settings.child(
            "pdshg_experiment", "power_control", "use_eom_calibration"
        ).value():
            eom_cal_file = self.settings.child(
                "pdshg_experiment", "power_control", "eom_calibration_file"
            ).value()
    ...
    def validate_parameters(self):
        ...
        wl_start = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "wavelength_range", "start_nm"
        ).value()
        wl_stop = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "wavelength_range", "stop_nm"
        ).value()
        wl_step = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "wavelength_range", "step_nm"
        ).value()
        ...
        p_min = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "power_range", "min_power"
        ).value()
        p_max = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "power_range", "max_power"
        ).value()
        p_points = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "power_range", "power_points"
        ).value()
        ...
        # Check angle range
        angle_start = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "angle_range", "start_angle"
        ).value()
        angle_stop = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "angle_range", "stop_angle"
        ).value()
        angle_step = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "angle_range", "angle_step"
        ).value()
        ...
        h800_formula = self.settings.child(
            "pdshg_experiment", "hwp_control", "h800_formula"
        ).value()
        h400_formula = self.settings.child(
            "pdshg_experiment", "hwp_control", "h400_formula"
        ).value()
    ...
    def create_data_structures(self):
        ...
        descending = self.settings.child(
            "pdshg_experiment", "measurement_ranges", "power_range", "descending_order"
        ).value()
        ...
        try:
            fine_regions = self.settings.child(
                "pdshg_experiment",
                "measurement_ranges",
                "angle_range",
                "fine_angle_regions",
            ).value()
            fine_step = self.settings.child(
                "pdshg_experiment",
                "measurement_ranges",
                "angle_range",
                "fine_angle_step",
            ...
            for region_str in fine_regions.split(","):
                if "-" in region_str.strip():
                    ...
                    fine_angles = np.arange(
                        start_fine, stop_fine + fine_step, fine_step
                    )
        ...
        self.measurement_data = {
            ...
            "powers": powers,
            "angles": angles,
            "photodiode_voltage": np.zeros(
                (len(wavelengths), len(powers), len(angles))
            ),
            "power_meter_reading": np.zeros(
                (len(wavelengths), len(powers), len(angles))
            ),
        ...
        logger.info(
            f"Created data structures: {len(wavelengths)} wavelengths, "
    ...
    def setup_experiment(self, current_step):
        ...
        settling_time = self.settings.child(
            "pdshg_experiment", "hwp_control", "settling_time"
        ).value()
    ...
    def perform_pdshg_measurements(self, current_step):
        ...
        for wl_idx, wavelength in enumerate(wavelengths):
            ...
            bg_frequency = self.settings.child(
                "pdshg_experiment", "data_acquisition", "background_frequency"
            ).value()
            if bg_frequency == "per_wavelength" or (
                bg_frequency == "start_only" and wl_idx == 0
            ):
            ...
            for p_idx, power in enumerate(powers):
                ...
                for a_idx, angle in enumerate(angles):
                    ...
                    # Store results
                    self.measurement_data["photodiode_voltage"][
                        wl_idx, p_idx, a_idx
                    ] = measurement_result["pd_voltage"]
                    self.measurement_data["power_meter_reading"][
                        wl_idx, p_idx, a_idx
                    ] = measurement_result["pm_reading"]
                    self.measurement_data["timestamps"][wl_idx, p_idx, a_idx] = (
                        measurement_result["timestamp"]
                    )
                    self.measurement_data["h800_positions"][wl_idx, p_idx, a_idx] = (
                        measurement_result["h800_pos"]
                    )
                    self.measurement_data["h400_positions"][wl_idx, p_idx, a_idx] = (
                        measurement_result["h400_pos"]
                    )
                    self.measurement_data["measurement_quality"][
                        wl_idx, p_idx, a_idx
                    ] = measurement_result["quality"]

                    # Real-time analysis if enabled
                    if self.settings.child(
                        "pdshg_experiment", "analysis_settings", "real_time_fitting"
                    ).value():
                    ...
                    logger.debug(
                        f"λ={wavelength:.1f}nm, P={power:.6f}W, θ={angle:.1f}° -> "
    ...
    def measure_background(self, wl_idx, current_step):
        ...
        for a_idx, angle in enumerate(angles):
            ...
            self.background_data["background_voltage"][wl_idx, a_idx] = bg_result[
                "pd_voltage"
            ]
            self.background_data["background_timestamps"][wl_idx, a_idx] = bg_result[
                "timestamp"
            ]
        ...
        logger.info(
            f"Background measurement completed for wavelength {self.measurement_data['wavelengths'][wl_idx]:.1f} nm"
        )
        return current_step
    ...
    def set_laser_power(self, target_power, current_step):
        ...
        if self.settings.child(
            "pdshg_experiment", "power_control", "power_verification"
        ).value():
            power_meter = self.modules["Newport1830C"]
            ...
            tolerance = self.settings.child(
                "pdshg_experiment", "power_control", "power_tolerance"
            ).value()
            error_percent = abs(power_value - target_power) / target_power * 100

            if error_percent > tolerance:
                logger.warning(
                    f"Power error {error_percent:.1f}% exceeds tolerance {tolerance}%"
                )
    ...
    def set_rotator_positions(self, angle, current_step, update_progress=True):
        ...
        h800_offset = self.settings.child(
            "pdshg_experiment", "hwp_control", "hwp_offsets", "h800_offset"
        ).value()
        h400_offset = self.settings.child(
            "pdshg_experiment", "hwp_control", "hwp_offsets", "h400_offset"
        ).value()
    ...
    def perform_single_measurement(self):
        ...
        averaging_samples = self.settings.child(
            "pdshg_experiment", "data_acquisition", "averaging_samples"
        ).value()
        timeout = self.settings.child(
            "pdshg_experiment", "data_acquisition", "measurement_timeout"
        ).value()
    ...
    def perform_real_time_analysis(self, wl_idx, p_idx, a_idx):
        ...
        voltages = self.measurement_data["photodiode_voltage"][
            wl_idx, p_idx, : a_idx + 1
        ]

        # Subtract background if available
        if self.settings.child(
            "pdshg_experiment", "data_acquisition", "background_subtraction"
        ).value():
            if wl_idx < len(self.background_data["background_voltage"]):
                bg_voltages = self.background_data["background_voltage"][
                    wl_idx, : a_idx + 1
                ]
        ...
        try:
            fit_func = self.settings.child(
                "pdshg_experiment", "analysis_settings", "angular_fit_function"
            ).value()

            if fit_func == "cos_squared_2theta":
                popt, pcov = curve_fit(
                    rashg_cosine_squared,
                    angles,
                    corrected_voltages,
                ...
                r_squared = 1 - np.sum(residuals**2) / np.sum(
                    (corrected_voltages - np.mean(corrected_voltages)) ** 2
                )
                ...
                logger.debug(
                    f"Real-time fit: A={amplitude:.4f}, φ={phase:.1f}°, R²={r_squared:.3f}"
                )
        ...
        for wl_idx, wavelength in enumerate(wavelengths):
            for p_idx, power in enumerate(powers):
                ...
                try:
                    ...
                    logger.warning(
                        f"Angular fitting failed for λ={wavelength}nm, P={power}W: {e}"
                    )

        # Power law analysis for each wavelength and angle
        if self.settings.child(
            "pdshg_experiment", "analysis_settings", "power_law_fitting"
        ).value():
            for wl_idx, wavelength in enumerate(wavelengths):
                for a_idx, angle in enumerate(angles):
                    # Get power dependence data
                    power_data = powers
                    voltage_data = self.measurement_data["photodiode_voltage"][
                        wl_idx, :, a_idx
                    ]
                    ...
                    if self.settings.child(
                        ...
                        if wl_idx < len(self.background_data["background_voltage"]):
                            bg_voltage = self.background_data["background_voltage"][
                                wl_idx, a_idx
                            ]
                    ...
                    # Fit power law
                    try:
                        expected_exp = self.settings.child(
                            "pdshg_experiment",
                            "analysis_settings",
                            "expected_shg_exponent",
                        ).value()
                        p0 = [1.0, expected_exp, 0.0]  # coefficient, exponent, offset

                        popt, pcov = curve_fit(
                            power_law_fitting, power_data, corrected_voltage_data, p0=p0
                        )
                        ...
                        r_squared = 1 - np.sum(residuals**2) / np.sum(
                            (corrected_voltage_data - np.mean(corrected_voltage_data))
                            ** 2
                        )
                        ...
                        logger.warning(
                            f"Power law fitting failed for λ={wavelength}nm, θ={angle}°: {e}"
                        )

        # Summary statistics
        angular_r_squared_values = [
            fit["r_squared"] for fit in self.angular_fits.values()
        ]
        power_law_r_squared_values = [
            fit["r_squared"] for fit in self.power_law_fits.values()
        ]

        logger.info(
            f"Angular fits: {len(self.angular_fits)} fits, "
            f"mean R² = {np.mean(angular_r_squared_values):.3f}"
        )
        logger.info(
            f"Power law fits: {len(self.power_law_fits)} fits, "
    ...
    def save_pdshg_results(self):
        ...
        try:
            with h5py.File(self.data_filepath, "w") as f:
                ...
                conditions_group.attrs["sample_name"] = self.settings.child(
                    "pdshg_experiment", "experimental_conditions", "sample_name"
                ).value()
                conditions_group.attrs["measurement_position"] = self.settings.child(
                    "pdshg_experiment",
                    "experimental_conditions",
                    "measurement_position",
                ).value()
                conditions_group.attrs["polarization_config"] = self.settings.child(
                    "pdshg_experiment", "experimental_conditions", "polarization_config"
                ).value()

                # Save hardware configuration
                hw_group = f.create_group("hardware_configuration")
                hw_group.attrs["h800_formula"] = self.settings.child(
                    "pdshg_experiment", "hwp_control", "h800_formula"
                ).value()
                hw_group.attrs["h400_formula"] = self.settings.child(
                    "pdshg_experiment", "hwp_control", "h400_formula"
                ).value()
                hw_group.attrs["h800_offset"] = self.settings.child(
                    "pdshg_experiment", "hwp_control", "hwp_offsets", "h800_offset"
                ).value()
                hw_group.attrs["h400_offset"] = self.settings.child(
                    "pdshg_experiment", "hwp_control", "hwp_offsets", "h400_offset"
                ).value()
