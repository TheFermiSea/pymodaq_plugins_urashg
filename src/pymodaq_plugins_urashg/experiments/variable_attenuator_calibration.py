...
class VariableAttenuatorCalibrationExperiment(URASHGBaseExperiment):
    ...
    required_modules = ['MaiTai', 'Newport1830C']  # Attenuator module TBD based on hardware
            ...
            'children': [
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'attenuator_type', 'type': 'list',
                         'limits': ['hwp_polarizer', 'motorized_nd', 'liquid_crystal'],
                         'value': 'hwp_polarizer'},
                        {'name': 'rotation_module', 'type': 'str', 'value': 'H800',
                         'tip': 'PyMoDAQ module controlling attenuator rotation'},
                        {'name': 'full_rotation_range', 'type': 'bool', 'value': True,
                ...
                },
                {
                    ...
                    'children': [
                        ...
                        {'name': 'angle_step', 'type': 'float', 'value': 2.0,
                         'limits': [0.1, 45], 'suffix': '°'},
                        {'name': 'high_resolution_regions', 'type': 'str',
                         'value': '85-95, 175-185, 265-275',
                         'tip': 'High-resolution angle ranges around minima'},
                        {'name': 'hr_angle_step', 'type': 'float', 'value': 0.2,
                         'limits': [0.01, 2.0], 'suffix': '°'},
                    ]
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'averaging_samples', 'type': 'int', 'value': 20,
                         'limits': [5, 200]},
                        {'name': 'measurement_timeout', 'type': 'float', 'value': 1.0,
                         'limits': [0.1, 10.0], 'suffix': 's'},
                        {'name': 'min_detectable_power', 'type': 'float', 'value': 1e-9,
                         'limits': [1e-12, 1e-6], 'suffix': 'W'},
                        {'name': 'power_stability_check', 'type': 'bool', 'value': True},
                        {'name': 'stability_tolerance', 'type': 'float', 'value': 1.0,
                ...
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'enable_wavelength_cal', 'type': 'bool', 'value': True},
                        {'name': 'wavelength_range', 'type': 'group',
                         'children': [
                             {'name': 'start_nm', 'type': 'float', 'value': 750.0,
                              'limits': [700, 950], 'suffix': 'nm'},
                             {'name': 'stop_nm', 'type': 'float', 'value': 850.0,
                              'limits': [750, 1000], 'suffix': 'nm'},
                             {'name': 'step_nm', 'type': 'float', 'value': 20.0,
                              'limits': [5, 100], 'suffix': 'nm'},
                         ]},
                        {'name': 'reference_angles', 'type': 'str',
                         'value': '0, 30, 45, 60, 90',
                ...
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'target_attenuations_db', 'type': 'str',
                         'value': '0.1, 0.5, 1, 2, 5, 10, 15, 20, 25, 30, 35',
                         'tip': 'Target attenuation levels for lookup table'},
                        {'name': 'attenuation_tolerance_db', 'type': 'float', 'value': 0.1,
                         'limits': [0.01, 1.0], 'suffix': 'dB'},
                        {'name': 'max_iterations', 'type': 'int', 'value': 10,
                         'limits': [3, 50]},
                    ]
                },
                {
                    ...
                    'type': 'group',
                    'children': [
                        {'name': 'fit_function', 'type': 'list',
                         'limits': ['malus_law', 'enhanced_malus', 'polynomial'],
                         'value': 'enhanced_malus'},
                        {'name': 'include_extinction_ratio', 'type': 'bool', 'value': True},
                        {'name': 'initial_extinction_ratio', 'type': 'float', 'value': 1e-4,
                         'limits': [1e-6, 1e-2]},
                        {'name': 'fit_tolerance', 'type': 'float', 'value': 1e-8,
                         'limits': [1e-12, 1e-4]},
                        {'name': 'max_fit_iterations', 'type': 'int', 'value': 2000,
                         'limits': [100, 10000]},
                    ]
                },
                {
                    ...
                    'children': [
                        ...
                        {'name': 'validation_attenuations_db', 'type': 'str',
                         'value': '0.7, 3, 7, 12, 18, 25',
                         'tip': 'Attenuation levels for validation'},
                        {'name': 'repeatability_tests', 'type': 'int', 'value': 5,
                         'limits': [2, 20]},
                        {'name': 'max_error_db', 'type': 'float', 'value': 0.2,
                         'limits': [0.05, 2.0], 'suffix': 'dB'},
    ...
    def initialize_specific_hardware(self):
        ...
        rotation_module = self.settings.child('attenuator_calibration', 'hardware_settings', 'rotation_module').value()
        if rotation_module not in self.modules:
            raise ExperimentError(f"Attenuator rotation module {rotation_module} not available")
    ...
    def validate_parameters(self):
        ...
        start_angle = self.settings.child('attenuator_calibration', 'angle_sweep', 'start_angle').value()
        stop_angle = self.settings.child('attenuator_calibration', 'angle_sweep', 'stop_angle').value()
        angle_step = self.settings.child('attenuator_calibration', 'angle_sweep', 'angle_step').value()
        ...
        if (stop_angle - start_angle) / angle_step > 1000:
            logger.warning("Very large number of angle points - this may take a long time")
        
        # Check wavelength parameters if enabled
        if self.settings.child('attenuator_calibration', 'wavelength_calibration', 'enable_wavelength_cal').value():
            wl_start = self.settings.child('attenuator_calibration', 'wavelength_calibration', 'wavelength_range', 'start_nm').value()
            wl_stop = self.settings.child('attenuator_calibration', 'wavelength_calibration', 'wavelength_range', 'stop_nm').value()
            wl_step = self.settings.child('attenuator_calibration', 'wavelength_calibration', 'wavelength_range', 'step_nm').value()
        ...
        try:
            target_att_str = self.settings.child('attenuator_calibration', 'attenuation_mapping', 'target_attenuations_db').value()
    ...
    def create_data_structures(self):
        ...
        hr_regions_str = self.settings.child('attenuator_calibration', 'angle_sweep', 'high_resolution_regions').value()
        hr_step = self.settings.child('attenuator_calibration', 'angle_sweep', 'hr_angle_step').value()
        ...
        try:
            ...
            logger.warning("Invalid high-resolution regions format, using regular step only")
        ...
        if self.settings.child('attenuator_calibration', 'wavelength_calibration', 'enable_wavelength_cal').value():
            ...
            ref_angles_str = self.settings.child('attenuator_calibration', 'wavelength_calibration', 'reference_angles').value()
        ...
        validation_steps = 0
        if self.settings.child('attenuator_calibration', 'validation', 'validate_calibration').value():
            val_att_str = self.settings.child('attenuator_calibration', 'validation', 'validation_attenuations_db').value()
            val_attenuations = [float(x.strip()) for x in val_att_str.split(',')]
            repeatability = self.settings.child('attenuator_calibration', 'validation', 'repeatability_tests').value()
            validation_steps = len(val_attenuations) * repeatability
            
            if self.settings.child('attenuator_calibration', 'validation', 'hysteresis_test').value():
        ...
        self.total_steps = angle_steps + wavelength_steps + validation_steps + mapping_steps + 10  # Extra for setup
        
        logger.info(f"Created data structures: {len(self.measurement_angles)} angle points")
    ...
    def measure_reference_power(self, current_step):
        ...
        settling_time = self.settings.child('attenuator_calibration', 'hardware_settings', 'settling_time').value()
        time.sleep(settling_time)
        
        # Measure reference power with averaging
        power_meter = self.modules['Newport1830C']
        averaging_samples = self.settings.child('attenuator_calibration', 'power_measurement', 'averaging_samples').value()
        ...
        logger.info(f"Reference power: {self.reference_power:.6f} ± {reference_std:.6f} W")
        self.log_status(f"Reference power measured: {self.reference_power:.6f} W")
        
        # Check power stability
        stability_tolerance = self.settings.child('attenuator_calibration', 'power_measurement', 'stability_tolerance').value()
        stability_percent = (reference_std / self.reference_power) * 100
        
        if stability_percent > stability_tolerance:
            logger.warning(f"Power stability {stability_percent:.2f}% exceeds tolerance {stability_tolerance}%")
    ...
    def perform_angle_sweep(self, current_step):
        ...
        settling_time = self.settings.child('attenuator_calibration', 'hardware_settings', 'settling_time').value()
        averaging_samples = self.settings.child('attenuator_calibration', 'power_measurement', 'averaging_samples').value()
        min_detectable_power = self.settings.child('attenuator_calibration', 'power_measurement', 'min_detectable_power').value()
        
        for i, angle in enumerate(self.measurement_angles):
            if self._check_stop_requested():
                return current_step
                
            self.log_status(f"Angle sweep: {angle:.1f}° ({i+1}/{len(self.measurement_angles)})")
            ...
            if self.reference_power > 0 and avg_power > 0:
                attenuation_db = calculate_attenuation_db(self.reference_power, avg_power)
            else:
            ...
            logger.debug(f"Angle {angle:.1f}° -> Power {avg_power:.6f}W, Attenuation {attenuation_db:.2f}dB")
    ...
    def fit_attenuation_model(self):
        ...
        fit_function_name = self.settings.child('attenuator_calibration', 'fitting_settings', 'fit_function').value()
        include_extinction = self.settings.child('attenuator_calibration', 'fitting_settings', 'include_extinction_ratio').value()
        
        if fit_function_name == 'enhanced_malus' and include_extinction:
            extinction_ratio = self.settings.child('attenuator_calibration', 'fitting_settings', 'initial_extinction_ratio').value()
            initial_guess = [amplitude_guess, phase_guess, offset_guess, extinction_ratio]
            fit_function = attenuator_malus_law
        else:
            initial_guess = [amplitude_guess, phase_guess, offset_guess]
            fit_function = lambda angle, amp, phase, offset: attenuator_malus_law(angle, amp, phase, offset, 1e-4)
        
        try:
            # Perform curve fitting
            max_iterations = self.settings.child('attenuator_calibration', 'fitting_settings', 'max_fit_iterations').value()
            tolerance = self.settings.child('attenuator_calibration', 'fitting_settings', 'fit_tolerance').value()
            
            popt, pcov = curve_fit(
                fit_function, fit_angles, fit_powers,
                p0=initial_guess,
            ...
            r_squared = 1 - np.sum(valid_residuals**2) / np.sum((fit_powers - np.mean(fit_powers))**2)
            
            # Store fit results
            if len(popt) == 4:
                amplitude, phase, offset, extinction_ratio = popt
                amplitude_err, phase_err, offset_err, extinction_err = np.sqrt(np.diag(pcov))
            ...
            max_attenuation_db = np.max(self.angle_sweep_data['attenuations_db'][np.isfinite(self.angle_sweep_data['attenuations_db'])])
            min_attenuation_db = np.min(self.angle_sweep_data['attenuations_db'][np.isfinite(self.angle_sweep_data['attenuations_db'])])
            
            self.fit_results['dynamic_range_db'] = max_attenuation_db - min_attenuation_db
            ...
            if 'extinction_ratio' in self.fit_results:
                logger.info(f"  Extinction ratio: {self.fit_results['extinction_ratio']:.2e}")
            logger.info(f"  R²: {r_squared:.6f}")
            logger.info(f"  Dynamic range: {self.fit_results['dynamic_range_db']:.1f} dB")
    ...
    def generate_attenuation_lookup_tables(self, current_step):
        ...
        if not hasattr(self, 'fit_results') or not self.fit_results.get('fit_success', False):
            raise ExperimentError("Cannot generate lookup tables - model fitting failed")
        
        target_att_str = self.settings.child('attenuator_calibration', 'attenuation_mapping', 'target_attenuations_db').value()
        target_attenuations = [float(x.strip()) for x in target_att_str.split(',')]
        tolerance_db = self.settings.child('attenuator_calibration', 'attenuation_mapping', 'attenuation_tolerance_db').value()
        max_iterations = self.settings.child('attenuator_calibration', 'attenuation_mapping', 'max_iterations').value()
        ...
        if self.fit_results['fit_function'] == 'enhanced_malus':
            powers_hr = attenuator_malus_law(angles_hr,
        ...
        attenuations_hr = np.array([calculate_attenuation_db(self.reference_power, p) for p in powers_hr])
        ...
        for target_att in target_attenuations:
            ...
            if target_att > self.fit_results['max_attenuation_db']:
                logger.warning(f"Target attenuation {target_att:.1f} dB exceeds maximum achievable {self.fit_results['max_attenuation_db']:.1f} dB")
                ...
                logger.warning(f"Target attenuation {target_att:.1f} dB below minimum achievable {self.fit_results['min_attenuation_db']:.1f} dB")
            ...
            if self.fit_results['fit_function'] == 'enhanced_malus':
                achieved_power = attenuator_malus_law(best_angle,
                                                    self.fit_results['amplitude'],
                                                    self.fit_results['phase'],
                                                    self.fit_results['offset'],
                                                    self.fit_results['extinction_ratio'])
            ...
            self.attenuation_lookup_tables['achieved_attenuations_db'].append(achieved_att)
            self.attenuation_lookup_tables['powers'].append(achieved_power)
            self.attenuation_lookup_tables['errors_db'].append(error_db)
            
            logger.info(f"Target {target_att:.1f} dB -> Angle {best_angle:.2f}°, "
        ...
        for key in ['angles', 'achieved_attenuations_db', 'powers', 'errors_db']:
            self.attenuation_lookup_tables[key] = np.array(self.attenuation_lookup_tables[key])
    ...
    def calibrate_wavelength_dependence(self, current_step):
        ...
        for wl_idx, wavelength in enumerate(wavelengths):
            ...
            for angle_idx, angle in enumerate(ref_angles):
                ...
                self.wavelength_dependent_data['attenuations'][wl_idx, angle_idx] = attenuation_db
    ...
    def validate_calibration_accuracy(self, current_step):
        ...
        repeatability_tests = self.settings.child('attenuator_calibration', 'validation', 'repeatability_tests').value()
        max_error_db = self.settings.child('attenuator_calibration', 'validation', 'max_error_db').value()
        hysteresis_test = self.settings.child('attenuator_calibration', 'validation', 'hysteresis_test').value()
        ...
        for target_att in validation_attenuations:
            ...
            if hasattr(self, 'angle_for_attenuation'):
                ...
                idx = np.argmin(np.abs(self.attenuation_lookup_tables['achieved_attenuations_db'] - target_att))
            ...
            for rep in range(repeatability_tests):
                ...
                measured_att = calculate_attenuation_db(self.reference_power, power_value)
            ...
            logger.info(f"Validation: Target {target_att:.1f}dB -> "
        ...
        self.validation_results = {
            ...
            'summary': {
                ...
                'mean_repeatability_db': np.mean([r['repeatability_db'] for r in validation_results])
            }
        }
        
        summary = self.validation_results['summary']
        logger.info(f"Validation summary: {summary['passed_points']}/{summary['total_points']} points passed")
        logger.info(f"Mean error: {summary['mean_error_db']:.3f} dB, Max error: {summary['max_error_db']:.3f} dB")
        ...
        try:
            with h5py.File(self.data_filepath, 'w') as f:
                ...
                if hasattr(self, 'validation_results'):
                    ...
                    for i, result in enumerate(self.validation_results['validation_points']):
                ...
                config_group.attrs['rotation_module'] = self.settings.child('attenuator_calibration', 'hardware_settings', 'rotation_module').value()
            ...
            logger.info(f"Variable attenuator calibration results saved to {self.data_filepath}")
    ...
    def get_attenuation_for_angle(self, angle):
        ...
        try:
            ...
            if self.fit_results['fit_function'] == 'enhanced_malus':
                power = attenuator_malus_law(angle,
                                           self.fit_results['amplitude'],
                                           self.fit_results['phase'],
                                           self.fit_results['offset'],
                                           self.fit_results['extinction_ratio'])
