...
def test_plugin_entry_points_exist():
    ...
    assert len(urashg_move) >= 3, f"Expected at least 3 move plugins, found {len(urashg_move)}"
    assert len(urashg_viewer) >= 2, f"Expected at least 2 viewer plugins, found {len(urashg_viewer)}"
...
def test_move_plugin_entry_points_loadable():
    ...
    for plugin_name in expected_plugins:
        if hasattr(eps, 'select'):
            ...
            ep_list = [ep for ep in eps.get('pymodaq.move_plugins', []) if ep.name == plugin_name]
...
def test_viewer_plugin_entry_points_loadable():
    ...
    for plugin_name in expected_plugins:
        if hasattr(eps, 'select'):
            ...
            ep_list = [ep for ep in eps.get('pymodaq.viewer_plugins', []) if ep.name == plugin_name]
