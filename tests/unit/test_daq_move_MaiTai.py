...
def mock_maitai_controller():
    ...
    with patch('pymodaq_plugins_urashg.hardware.urashg.maitai_control.MaiTaiController') as mock_controller:
...
def test_ini_stage(maitai_plugin):
    ...
    maitai_plugin.settings.child('connection_group', 'serial_port').setValue('/dev/ttyUSB0')
