...
# Test markers
pytestmark = [pytest.mark.camera, pytest.mark.integration]
...
class TestPrimeBSIMockMode:
    ...
    def test_parameter_structure(self, camera_plugin):
        ...
        param_names = [
            p.get("name") for p in camera_plugin.params if isinstance(p, dict)
        ]
        required_groups = [
            "camera_settings",
            "advanced_settings",
    ...
    def test_camera_parameters_population(self, camera_plugin):
        ...
        camera_name = camera_plugin.settings.child(
            "camera_settings", "camera_name"
        ).value()
        sensor_size = camera_plugin.settings.child(
            "camera_settings", "sensor_size"
        ).value()
    ...
    def test_temperature_monitoring(self, camera_plugin):
        ...
        temp_param = camera_plugin.settings.child(
            "camera_settings", "temperature_setpoint"
        )
        ...
        try:
            ...
            if plugin.initialized:
                ...
                for exp_time in exposures:
                    plugin.settings.child("camera_settings", "exposure").setValue(
                        exp_time
                    )
                    plugin.grab_data()
