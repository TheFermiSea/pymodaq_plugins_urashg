...
class MockNewport1830CController:
    ...
    def set_power_range(self, power_range: str) -> bool:
        ...
        if not self._connected:
            return False
        valid_ranges = ["Auto", "Range 1", "Range 2", "Range 3", 
...
def newport_plugin(mock_serial_environment, mock_pymodaq_environment):
    ...
    def mock_ini_detector(controller=None):
        ...
        try:
            plugin.emit_status(
                plugin.__class__.__bases__[0].__dict__.get('ThreadCommand', lambda x, y: None)("show_splash", "Initializing Newport 1830-C...")
            )
            ...
            plugin.emit_status(plugin.__class__.__bases__[0].__dict__.get('ThreadCommand', lambda x, y: None)("close_splash"))
            
            info_string = "Newport 1830-C initialized on /dev/ttyS0"
            plugin.emit_status(plugin.__class__.__bases__[0].__dict__.get('ThreadCommand', lambda x, y: None)("Update_Status", [info_string]))
...
class TestNewportPowerMeterMockMode:
    ...
    def test_plugin_import(self, mock_serial_environment, mock_pymodaq_environment):
        ...
        with patch('pymodaq_plugins_urashg.daq_viewer_plugins.plugins_0D.daq_0Dviewer_Newport1830C.Newport1830CController', MockNewport1830CController):
            from pymodaq_plugins_urashg.daq_viewer_plugins.plugins_0D.daq_0Dviewer_Newport1830C import DAQ_0DViewer_Newport1830C
    ...
    def test_parameter_structure(self, newport_plugin):
        ...
        param_names = [p.get('name') for p in newport_plugin.params if isinstance(p, dict)]
        required_groups = ['connection_group', 'measurement_group', 'calibration_group', 'status_group']
    ...
    def test_measurement_parameters(self, newport_plugin):
        ...
        wavelength_param = newport_plugin.settings.child('measurement_group', 'wavelength')
    ...
    def test_wavelength_calibration(self, newport_plugin):
        ...
        for wavelength in test_wavelengths:
            ...
            current_wl = newport_plugin.settings.child('measurement_group', 'wavelength').value()
    ...
    def test_status_updates(self, newport_plugin):
        ...
        device_status = newport_plugin.settings.child('status_group', 'device_status').value()
        ...
        try:
            ...
            if success:
                ...
                for avg_count in [1, 3, 5]:
                    plugin.settings.child('measurement_group', 'averaging').setValue(avg_count)
