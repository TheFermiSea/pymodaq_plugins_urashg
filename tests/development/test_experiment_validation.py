...
def test_experiment_imports():
    ...
    experiments = [
        ("Base Experiment", "src.pymodaq_plugins_urashg.experiments.base_experiment", "URASHGBaseExperiment"),
        ("Elliptec Calibration", "src.pymodaq_plugins_urashg.experiments.elliptec_calibration", "ElliptecCalibrationExperiment"),
        ("EOM Calibration", "src.pymodaq_plugins_urashg.experiments.eom_calibration", "EOMCalibrationExperiment"),
        ("PDSHG Experiment", "src.pymodaq_plugins_urashg.experiments.pdshg_experiment", "PDSHGExperiment"),
        ("Variable Attenuator", "src.pymodaq_plugins_urashg.experiments.variable_attenuator_calibration", "VariableAttenuatorCalibrationExperiment"),
        ("Basic μRASHG", "src.pymodaq_plugins_urashg.experiments.basic_urashg_experiment", "BasicURASHGExperiment"),
        ("Wavelength Dependent", "src.pymodaq_plugins_urashg.experiments.wavelength_dependent_rashg", "WavelengthDependentRASHGExperiment"),
    ]
...
def test_experiment_instantiation():
    ...
    try:
        from src.pymodaq_plugins_urashg.experiments.elliptec_calibration import ElliptecCalibrationExperiment
        ...
        if hasattr(experiment, 'settings') and experiment.settings:
            print(f"✅ Parameter tree created with {len(experiment.settings.children())} main groups")
            
            # Check main parameter groups
            expected_groups = ['experiment_info', 'file_settings', 'hardware_settings', 'calibration_settings', 'safety_settings']
...
def test_experiment_parameters():
    ...
    try:
        ...
        if hasattr(experiment, 'settings'):
            ...
            wavelength_param = experiment.settings.child('hardware_settings', 'maitai_wavelength')
            if wavelength_param:
                print(f"✅ Wavelength parameter: {wavelength_param.value()} nm")
                print(f"   Range: {wavelength_param.opts.get('limits', 'No limits set')}")
            
            # Test calibration settings
            calib_group = experiment.settings.child('calibration_settings')
            if calib_group:
                print(f"✅ Calibration settings group with {len(calib_group.children())} parameters")
...
def test_experiment_methods():
    ...
    try:
        ...
        from src.pymodaq_plugins_urashg.experiments.base_experiment import ExperimentState
        ...
        try:
            ...
            print("✅ Parameter validation method correctly raises NotImplementedError (to be overridden)")
        except Exception as e:
        ...
        try:
            ...
            print("✅ Data structure creation correctly raises NotImplementedError (to be overridden)")
        except Exception as e:
