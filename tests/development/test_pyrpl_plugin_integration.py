...
def test_plugin_discovery():
    ...
    try:
        ...
        pyrpl_move_plugins = [name for name, ep in move_plugins.items() 
        ...
        pyrpl_viewer_plugins = [name for name, ep in viewer_plugins.items() 
                               if 'pyrpl' in ep.module_name.lower()]
...
def test_pyrpl_library_import():
    ...
    try:
        import pyrpl
        logger.info(f"✅ PyRPL library imported successfully (version: {getattr(pyrpl, '__version__', 'unknown')})")
...
def test_urashg_experiment_integration():
    ...
    try:
        from pymodaq_plugins_urashg.experiments.wavelength_dependent_rashg import WavelengthDependentRASHGExperiment
        ...
        if 'PyRPL_PID' in required_modules:
            logger.info("✅ URASHG experiment correctly references external PyRPL_PID plugin")
            
            if 'URASHG_PyRPL_PID' not in required_modules:
                logger.info("✅ Internal URASHG_PyRPL_PID plugin correctly removed from requirements")
            else:
                logger.warning("⚠️  Internal URASHG_PyRPL_PID still referenced - potential conflict")
