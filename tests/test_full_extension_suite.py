...
class TestExtensionSuiteOverview:
    ...
    def test_pymodaq_framework_available(self):
        ...
        try:
            ...
            logger.info(f"✓ PyMoDAQ framework available (version: {getattr(pymodaq, '__version__', 'unknown')})")
        except ImportError as e:
...
class TestExtensionDiscoveryCompliance:
    ...
    def test_extension_entry_point_registered(self):
        ...
        urashg_extensions = [ep for ep in extension_eps
        ...
        logger.info(f"✓ Found {len(urashg_extensions)} URASHG extension(s) in entry points")
    ...
    def test_all_plugin_entry_points_registered(self):
        ...
        for group, plugin_names in expected_plugins.items():
            ...
            assert len(missing_plugins) == 0, f"Missing plugins in {group}: {missing_plugins}"
...
class TestExtensionArchitectureCompliance:
    ...
    def test_extension_inheritance_pattern(self, extension_class):
        ...
        assert issubclass(extension_class, CustomApp), "Extension should inherit from CustomApp"
    ...
    def test_extension_required_metadata(self, extension_class):
        ...
        for attr in required_attrs:
            assert hasattr(extension_class, attr), f"Extension missing required attribute: {attr}"
    ...
    def test_extension_signal_architecture(self, extension_class):
        ...
        required_signals = [
            'measurement_started', 'measurement_finished', 'measurement_progress',
            'device_status_changed', 'error_occurred'
        ]

        # Check that signal attributes are defined at class level
        for signal_name in required_signals:
            assert hasattr(extension_class, signal_name), f"Missing signal: {signal_name}"
    ...
    def test_extension_parameter_tree_compliance(self, extension_class):
        ...
        assert hasattr(extension_class, 'params'), "Extension should have params class attribute"
...
class TestDeviceManagerCompliance:
    ...
    def test_device_manager_architecture(self):
        ...
        assert issubclass(URASHGDeviceManager, QObject), "DeviceManager should inherit from QObject"
    ...
    def test_device_manager_signal_patterns(self):
        ...
        with patch('pymodaq_plugins_urashg.extensions.device_manager.URASHGDeviceManager'):
            mock_dashboard = Mock()
            dm = URASHGDeviceManager(mock_dashboard)

            # Check for expected signals
            expected_signals = ['device_status_changed', 'device_error_occurred', 'all_devices_ready']

            for signal_name in expected_signals:
                if hasattr(dm, signal_name):
                    signal_obj = getattr(dm, signal_name)
                    assert isinstance(signal_obj, Signal), f"{signal_name} should be Qt Signal"
    ...
    def test_device_status_management(self):
        ...
        expected_statuses = ['DISCONNECTED', 'CONNECTING', 'CONNECTED', 'READY', 'BUSY', 'ERROR']
...
class TestMeasurementWorkerCompliance:
    ...
    def test_measurement_worker_threading_compliance(self):
        ...
        assert issubclass(MeasurementWorker, QObject), "MeasurementWorker should inherit from QObject"
    ...
    def test_measurement_worker_lifecycle_methods(self):
        ...
        required_methods = ['run_measurement', 'stop_measurement', 'pause_measurement', 'resume_measurement']
    ...
    def test_measurement_worker_signals(self):
        ...
        signal_names = [attr for attr in dir(worker) if isinstance(getattr(worker, attr, None), Signal)]
...
class TestPluginIntegrationCompliance:
    ...
    def test_plugin_base_class_compliance(self):
        ...
        for plugin_name, module_path, expected_base in plugin_tests:
            try:
                ...
                assert issubclass(plugin_class, expected_base), f"{plugin_name} should inherit from {expected_base.__name__}"
    ...
    def test_plugin_method_compliance(self):
        ...
        move_methods = ['ini_attributes', 'get_actuator_value', 'close', 'commit_settings', 'ini_stage', 'move_abs']
        viewer_methods = ['ini_attributes', 'grab_data', 'close', 'commit_settings', 'ini_detector']
        ...
        try:
            ...
            for method_name in move_methods:
                assert hasattr(DAQ_Move_MaiTai, method_name), f"DAQ_Move_MaiTai missing {method_name}"
        ...
        try:
            ...
            for method_name in viewer_methods:
                assert hasattr(DAQ_2DViewer_PrimeBSI, method_name), f"DAQ_2DViewer_PrimeBSI missing {method_name}"
    ...
    def test_plugin_parameter_compliance(self):
        ...
        try:
            ...
            assert hasattr(DAQ_Move_MaiTai, 'params'), "Plugin should have params attribute"
...
class TestErrorHandlingCompliance:
    ...
    def test_extension_error_handling(self):
        ...
        with patch('pymodaq_plugins_urashg.extensions.urashg_microscopy_extension.URASHGDeviceManager'):
            ...
            assert hasattr(extension, 'on_error_occurred'), "Extension should handle errors"
            assert hasattr(extension, 'error_occurred'), "Extension should have error signal"
    ...
    def test_graceful_degradation(self):
        ...
        with patch('pymodaq_plugins_urashg.extensions.urashg_microscopy_extension.URASHGDeviceManager', side_effect=ImportError):
            try:
                from pymodaq_plugins_urashg.extensions.urashg_microscopy_extension import URASHGMicroscopyExtension
        ...
        with patch('pymodaq_plugins_urashg.extensions.urashg_microscopy_extension.URASHGDeviceManager'):
            ...
            for method_name in config_methods:
                if hasattr(extension, method_name):
                    assert callable(getattr(extension, method_name)), f"{method_name} should be callable"
...
class TestDocumentationCompliance:
    ...
    def test_extension_documentation(self):
        ...
        assert URASHGMicroscopyExtension.__doc__ is not None, "Extension should have docstring"
        assert len(URASHGMicroscopyExtension.__doc__.strip()) > 100, "Extension docstring should be comprehensive"
    ...
    def test_plugin_documentation(self):
        ...
        try:
            ...
            assert len(DAQ_Move_MaiTai.__doc__.strip()) > 50, "Plugin docstring should be descriptive"
